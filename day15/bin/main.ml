...
let parse_data input  = 
  let rec aux inp mp moves  =
    ...
    | hd :: tl ->  
      if (String.contains hd '#') then aux tl (hd :: mp) moves
      else aux tl mp (hd :: moves)
  in 
  let map_out, moves_out = aux input [] [] in
  let moves_final = String.concat "" moves_out in
  let map_final = Array.of_list(List.map (fun x -> split(x)|> (List.map parse_tile) |> Array.of_list)map_out ) in
...
let locate_robot grid =
  let i = Array.find_index (fun x -> Array.exists ( fun y -> y = Robot)x) grid in
    ...
    | Some idx -> idx
    | _ -> failwith "No robot" in
  let j = 
    Array.find_index (fun x -> x = Robot)grid.(idx) in
    match j with 
    | Some jdx -> idx, jdx
    | _ -> failwith "No robot"
...
let find_empty_tile (di, dj) (i,j) grid =
    let rec aux ni nj =
      if grid.(ni).(nj) = Wall then None
      else if grid.(ni).(nj) = Empty then Some(ni,nj)
      else aux (ni + di) (nj + dj) in
    aux i j

let pull_into (i,j) (ri, rj) grid =
  let rec aux ni nj =
      if grid.(ni).(nj) = Robot then(
    grid.(ni).(nj) <- Empty;
  grid)
      else 
...
let attempt_move (di, dj) (i, j) grid = 
  ...
  | Some(ni, nj) -> 
    (
      ((i+di, j+dj),pull_into (ni, nj) (-di, -dj) grid)
    )



let step loc map m = 
    let new_loc, new_map = match m with
...
let sum_gps mat = 
    ...
    Array.iteri (fun i row -> 
      Array.iteri (fun j tile -> 
        if tile = Box then sum := !sum + (100*i) + j) 
      row
  )
  mat;
!sum
...
let part1 data =
    ...
    let rec aux loc grid m = 
      if String.length m = 0 then grid
      else( 
...
let () =
  let data = read (open_in "data/input") in
  let answer = part1 data in
  Printf.printf "Part 1: %d" answer;
