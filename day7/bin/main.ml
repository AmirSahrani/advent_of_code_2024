...
let split_numbers data = List.map int_of_string (String.split_on_char ' ' (String.trim data))
...
let check_possibility (target, sums) ops =
  let rec check left rhs=
    if left > target then false else
    match rhs with
    | [] -> left = target
...
let () =
  ...
  let valid_part1 = time (List.filter (fun x -> check_possibility x  [( + );( * )])) fixed in
  let valid_part2 = time (List.filter (fun x -> check_possibility x  [( + );( * ); concat])) fixed in
